...
internal suspend fun <K, V> SmallD.post(
    ...
    UnitSerializer -> {
        ioCoroutineScope.launch { post(path,
            compile(payloadSerializer, payload), *attachments) }
...
internal suspend inline fun <K, V> SmallD.put(
    ...
    UnitSerializer -> {
        ioCoroutineScope.launch { put(path,
            compile(payloadSerializer, payload)
        ) }
        @Suppress("UNCHECKED_CAST") // we already checked due to the when statement limiting it.
        Unit as V
    }
...
internal suspend inline fun <K, V> SmallD.patch(
    ...
    UnitSerializer -> {
        ioCoroutineScope.launch { patch(path,
            compile(payloadSerializer, payload)
        ) }
        @Suppress("UNCHECKED_CAST") // we already checked due to the when statement limiting it.
        Unit as V
